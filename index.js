    ...
    new SMTPServer({
        ...
        async onData(stream, session, callback) {
            ...
            try {
                ...
                const fields = Object.entries(config.smtp.fields || {}).reduce((acc, [fieldName, fieldValue]) => {
                    ...
                    if (typeof fieldValue === 'object') {
                        ...
                        if (fieldValue.xfrm) {
                            const ctx = {
                                message: parsed,
                                value: fieldValue.value == true ? JSON.stringify(value) : String(value)
                            };
                            try {
                ...
                if (fields.$event) {
                    debug(`Publishing to ${join(prefix, 'events', fields.$event)}`);
                    mqtt.publish(join(prefix, 'events', fields.$event), 'true');
                }
                for (const [field, value] of Object.entries(fields)) {
                    if (field[0] === '$' || value === undefined) continue;
                    debug(`Publishing to ${join(prefix, 'fields', field)}`);
